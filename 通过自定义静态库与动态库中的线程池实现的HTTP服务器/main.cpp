...
#include "static_ThreadPool.h"

#pragma comment(lib, "ws2_32.lib")
#pragma comment(lib, "x64/pthreadVC2.lib")
//#pragma comment(lib, "static_ThreadPool.lib")
//#pragma comment(lib, "../动态库中的线程池/x64/Release/ThreadPool_DLL.lib")
//#pragma comment(lib, "static_ThreadPool.lib")

#define BUFF_SIZE 1024 
typedef ThreadPool* (*funPtr)(int);
...
int WaitConnect(SOCKET _ListenSocket)
{
	...
	/*
	链接动态库文件的方法
	HMODULE hDLL = LoadLibrary(L"../动态库中的线程池/x64/Release/ThreadPool_DLL.dll");
	if (hDLL == NULL)
	{
		std::cout << "动态库模块加载失败" << std::endl;
		return 0;
	}
	else
	{
		std::cout << "动态库模块地址为：" << hDLL << std::endl;
	}

	funPtr fun = (funPtr)GetProcAddress(hDLL, "getInstance");
	ThreadPool* pool = fun(7);*/
